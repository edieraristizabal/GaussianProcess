...
# ─────────────────────────────────────────────────────────────────────
print('STEP 7: Generating Fig11 (uncertainty vs covariates – Slope, Geology, Landcover)...')
LABEL_SIZE = 13
TICK_SIZE  = 11
TITLE_SIZE = 14

# Layout: 2 rows × 2 cols
#   Row 0: Slope hexbin spans both columns (full width)
#   Row 1: Geology boxplot (left) | Landcover boxplot (right)
...
# ── Row 0: Slope hexbin (spans both columns) ──────────────────────────
ax_slope = fig.add_subplot(gs[0, :])
x = df_sub['Slope'].values
y = df_sub['Std'].values
xlim = [0, 70]

hb = ax_slope.hexbin(x, y, gridsize=60, cmap='YlOrRd', mincnt=1, bins='log',
                     linewidths=0.15,
                     extent=[xlim[0], xlim[1], 0, y_global_max])
cb = fig.colorbar(hb, ax=ax_slope, pad=0.01, shrink=0.85)
cb.set_label('log₁₀(celdas)', fontsize=TICK_SIZE - 1)
cb.ax.tick_params(labelsize=TICK_SIZE - 1)

bins_s = np.linspace(xlim[0], xlim[1], 19)
mids_s = 0.5 * (bins_s[:-1] + bins_s[1:])
m_s, q25_s, q75_s = [], [], []
for lo, hi in zip(bins_s[:-1], bins_s[1:]):
    vals = y[(x >= lo) & (x < hi)]
    if len(vals) > 20:
        m_s.append(np.mean(vals)); q25_s.append(np.percentile(vals, 25))
        q75_s.append(np.percentile(vals, 75))
    else:
        m_s.append(np.nan); q25_s.append(np.nan); q75_s.append(np.nan)
m_s, q25_s, q75_s = np.array(m_s), np.array(q25_s), np.array(q75_s)
ok_s = ~np.isnan(m_s)
ax_slope.plot(mids_s[ok_s], m_s[ok_s], color='navy', lw=2.2, zorder=5, label='Media')
ax_slope.fill_between(mids_s[ok_s], q25_s[ok_s], q75_s[ok_s],
                      color='navy', alpha=0.20, zorder=4, label='IQR (25–75%)')
ax_slope.set_xlim(xlim); ax_slope.set_ylim(0, y_global_max)
ax_slope.set_xlabel('Pendiente (°)', fontsize=LABEL_SIZE)
ax_slope.set_ylabel('Desv. estándar posterior', fontsize=LABEL_SIZE)
ax_slope.tick_params(labelsize=TICK_SIZE)
ax_slope.legend(fontsize=TICK_SIZE - 1, loc='upper right', framealpha=0.8)
ax_slope.text(0.015, 0.97, next(letters), transform=ax_slope.transAxes,
              fontsize=TITLE_SIZE, fontweight='bold', va='top', ha='left',
              bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.85, ec='none'))

# ── Row 1 left: Geology boxplot ───────────────────────────────────────
...
fig.suptitle(
    'Estructura de la incertidumbre del PGC en el espacio de covariables del modelo óptimo',
...
# ─────────────────────────────────────────────────────────────────────
print('STEP 8: Generating Fig12 (PGC uncertainty vs susceptibility)...')

# ── Cargar susceptibilidad PGC ────────────────────────────────────────
with rasterio.open(str(DATA_DIR / 'susceptibility_GP_mean.tif')) as src:
...
# ── Figura single panel: PGC ─────────────────────────────────────────
fig12, ax12 = plt.subplots(figsize=(9, 7))

hb = ax12.hexbin(gpc_susc, gpc_std, gridsize=70, bins='log',
                 cmap='YlOrRd', mincnt=1, linewidths=0.2)
cx, mu, q25, q75 = bin_stats(gpc_susc, gpc_std)
ax12.plot(cx, mu, color='#cb181d', lw=2.5, zorder=5, label='Media binada')
ax12.fill_between(cx, q25, q75, color='#cb181d', alpha=0.28,
                  zorder=4, label='IQR (25–75%)')
ax12.axhline(float(np.mean(gpc_std)), color='grey', lw=1.2, ls='--', alpha=0.7,
             label=f'Media global ({np.mean(gpc_std):.3f})')

ax12.set_xlabel('Susceptibilidad PGC (probabilidad posterior)', fontsize=LABEL_SIZE + 1)
ax12.set_ylabel('Desviación estándar posterior (incertidumbre)', fontsize=LABEL_SIZE + 1)
ax12.set_xlim(0, 1)
ax12.tick_params(labelsize=TICK_SIZE)
ax12.legend(fontsize=TICK_SIZE, framealpha=0.85, loc='upper right')

cb12 = plt.colorbar(hb, ax=ax12, fraction=0.035, pad=0.03)
cb12.set_label('Celdas (log₁₀)', fontsize=TICK_SIZE)
cb12.ax.tick_params(labelsize=TICK_SIZE - 1)
