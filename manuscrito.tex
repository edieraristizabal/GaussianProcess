...
modelos PGR univariados y multivariados. Los modelos univariados revelan que las variables categóricas son las más informativas para
...
(AUC~$\approx$~0{,}550). Considerando que el aspecto no se;alo un patron diferencial entre celdas on y sisn deslizamientos y que ITH se correlaciona inversamente con la pendiente,
...
altos (susceptibilidad~$> 0{,}6$) se concentran en las laderas medias sobre suelso derivados de rocas metamórficas con pendientes escarpadas, 
...
covariables locales no tiene capacidad suficiente para predecir la densidad espacial regional del inventario. Desde una perspectiva conceptual, el PGR podría ser apropiado si el objetivo fuera predecir la densidad regional 
de eventos futuros en un contexto con datos ambientales y morfometricos a escala similar; para este caso mas local o puntual al parecer 
las variables morfometricas locales no explican adecuadamente la densidad de deslizamientos. Adicionalmente, la 
...
condicionante con mayor capacidad discriminativa. Este contraste senala que la importancia de una variable no es una propiedad
...
Los resultados senalan que el PG alcanza un desempeño competitivo con pocas variables y proporciona como producto nativo la distribución posterior 
...
La dominancia de la cobertura del suelo en el ARD (importancia relativa 0{,}272), sobre la pendiente (0{,}215) y la geología (0{,}062)
difiere del patrón reportado habitualmente en estudios regionales, donde la pendiente tiende a ser el predictor dominante \citep{Reichenbach2018,
Pradhan2010}. A mayor resolución el modelo distinge variaciones locales de la cobertura que reflejan directamente la intervención antrópica del suelo, 
deforestación, urbanización informal, condominios, cuya influencia sobre la susceptibilidad supera en esta cuenca
